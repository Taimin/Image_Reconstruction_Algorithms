...
voxels(:, 4) = zeros(size(voxels(:, 4)));%The fourth column of voxels change to zero
...
for i = 1:size(params,3)
    ...
    ind1 = find(object_points_cam(2,:)>img_size(1));%find the points in object_points_cam outside of the image size
    object_points_cam(:,ind1) = 1;%points over the size of image was set to 1,1,1
