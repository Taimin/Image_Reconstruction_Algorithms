...
fid = fopen([data_dir file_base '_par.txt'], 'r');%[] paste different section of words into one string
res = textscan(fid,'%d');
N = res{1,1};
for i=1:N
    textscan(fid,'%s',1);
...
for i=1:size(imgs,4)
     ch1 = imgs(:,:,1,i)>T;%because this is a color image
     ch2 = imgs(:,:,2,i)>T;
     ch3 = imgs(:,:,3,i)>T;
...
% [fv]  = isosurface(voxel3Dx, voxel3Dx, voxel3Dz, voxel3D, iso_value, voxel3Dz);
% [faces, verts, colors]  = isosurface(voxel3Dx, voxel3Dy, voxel3Dz, voxel3D, iso_value, voxel3Dz);
[fv]  = isosurface(voxel3Dx, voxel3Dz, voxel3Dy, voxel3D, iso_value, voxel3Dz);
[faces, verts, colors]  = isosurface(voxel3Dx, voxel3Dz, voxel3Dy, voxel3D, iso_value, voxel3Dz);
